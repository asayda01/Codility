...
function Tape_Equilibrium(A: number[]): number {
    
    if ( A.length === 2 ) {return Math.abs( A[0] - A[1] ) };

    let left_sum  :number = A[0];
    let right_sum :number = [...A].splice(1).reduce( (a,b) => a+b , 0 );
    let curr , min : number = Math.abs( left_sum -  right_sum ); 

    for ( let i of [...A].splice(1,A.length-1) ) {
